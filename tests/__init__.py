...
.. note::
  ...
  Unit tests for **OpenAPI for Python** are written using `pytest`_ and
...
**OpenAPI for Python**, we decided to focus on practicality. That means:
  ...
  * **Coverage matters...kind of.** We have documented the primary intended
    behavior of every function in **OpenAPI for Python**, and the
    ...
    necessary.
  * **Sample Documents Matter.** The test suite relies on an extended set of sample
    OpenAPI documents, some of which have been collected from real (open-source) API
    resources. The objective is to ensure that we have solid and demonstrable support
    for the entire
...
Each individual test module (e.g. ``test_OpenAPI.py``) corresponds to a
conceptual grouping of functionality. For example:

* ``test_OpenAPI.py`` tests the :class:`OpenAPI` class and its methods
...
**OpenAPI for Python** does not use any custom command-line options in its
test suite.
...
Because **OpenAPI for Python** has a very simple test suite, we have not
...
.. note::
  ...
  Because of the simplicity of **OpenAPI for Python**, the test suite does
...
.. note::
  ...
  The **OpenAPI for Python** test suite does support incremental testing using,
