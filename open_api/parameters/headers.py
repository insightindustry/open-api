...
class Headers(OpenAPIDict):
    """A container for generic named :class:`Header` instances that can be
    ...
    def _validate_value(self, value):
        """Internal method that validates that a value is a valid :class:`Header`,
        :class:`Reference`, or compatible :class:`dict <python:dict>` object.

        :param value: The value to validate.

        :returns: A :class:`Header` or :class:`Reference` object.
        :rtype: :class:`Header` / :class:`Reference`
        ...
        if checkers.is_dict(value):
            try:
                value = Header.new_from_dict(value)
            except ValueError:
                try:
                    value = Reference.new_from_dict(value)
                except ValueError:
                    raise ValueError('value must be a Header instance, '
                                     'Reference instance, or compatible dict '
                                     'but was: %s' % type(value))
        elif not checkers.is_type(value, ('Header', 'Reference')):
            raise ValueError('value must be a Header instance, '
                             'Reference instance, or compatible dict '
                             'but was: %s' % type(value))
